...
const PopupAds = (props: Props) => {
  ...
  return (
    <Grid item xs={6} md={4} sx={styles.gridContentWrapper}>
      ...
      <ImageList cols={1}>
        <ImageListItem onClick={props.openModal} sx={styles.imgWrapper} style={{ width: 300, height: 300 }}>				
          <img src={props.data?.photo_url} style={{ width: 300, height: 300, objectFit: "cover" }} alt='template icon' />
        </ImageListItem>
      </ImageList>
    </Grid>
  )
