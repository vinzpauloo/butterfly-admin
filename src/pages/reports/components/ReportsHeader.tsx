...
const ReportsHeader = (props: Props) => {
  ...
  const [timespan, setTimespan, fromDate, toDate, setFromDate, setToDate] = reportsHeaderStore(state => [
    state.timespan,
    state.setTimespan,
    state.fromDate,
    state.toDate,
    ...
    <DatePickerWrapper>
      <Stack gap={4} mb={4}>
        <Typography variant='h4' component='h4'>
          {TranslateString('Reports')} - {TranslateString(`${title}`)}
        </Typography>
        <Stack direction='row' gap={4} alignItems='flex-end'>
          ...
          <ToggleButtonGroup color='primary' value={timespan} exclusive onChange={handleOnChange} size='small'>
            <ToggleButton value='daily' sx={{ textTransform: 'capitalize' }}>
              Daily
            </ToggleButton>
            {/* <ToggleButton value='weekly' sx={{ textTransform: 'capitalize' }}>Weekly</ToggleButton> */}
            <ToggleButton value='monthly' sx={{ textTransform: 'capitalize' }}>
              Monthly
            </ToggleButton>
